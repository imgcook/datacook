...
export abstract class Criterion {
     ...
     */
    abstract init(y: number[], sampleWeight: number[], samples: number[], weightedNSamples: number, start: number, end: number): void;
    ...
    abstract proxyImpurityImprovement(): number
...
export abstract class RegressionCriterion extends Criterion {
    public sqSumTotal: number;
    /**
     * Initialize the criterion.
     * @param y The target stored as a buffer for memory efficiency
     * @param samples The weight of each sample
    ...
    public init(y: number[], sampleWeight: number[], samples: number[], weightedNSamples: number, start: number, end: number): void {
      ...
      for (let i = start; i < end; i++) {
        ...
        this.weightedNNodeSamples += w;
        this.sqSumTotal += w * y[pos] * y[pos];
        this.sumTotal[0] += w * y[pos];
      }
    }
    /**
     * Reset the criterion at pos=start.
    ...
    public update(samples: number[], newPos: number): void {
      ...
      // update right part statistics
      this.weightedNRight = this.weightedNNodeSamples - this.weightedNLeft;
      this.sumRight[0] = this.sumTotal[0] - this.sumLeft[0];
      this.pos = newPos;
    }
    /**
     * Compute the node value of samples[start:end] into dest.
     */
    public nodeValue(): number[] {
      return [ this.sumTotal[0] / this.weightedNNodeSamples ];
    }
}
...
export class MSECriterion extends RegressionCriterion {
  public nodeImpurity(): number {
    const val = this.sumTotal[0] / this.weightedNNodeSamples;
    return this.sqSumTotal / this.weightedNNodeSamples - val * val;
  }
  public proxyImpurityImprovement(): number {
    return this.sumLeft[0] * this.sumLeft[0] / this.weightedNLeft + this.sumRight[0] * this.sumRight[0] / this.weightedNRight;
  }
  public childrenImpurity(): { impurityLeft: number; impurityRight: number; } {
    let sqSumLeft = 0;
    let sqSumRight = 0;
    for (let p = this.start; p < this.pos; p++) {
    ...
    const impurityRight = sqSumRight / this.weightedNRight - rightVal * rightVal;
