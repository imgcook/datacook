...
import { PCA } from '@pipcook/datacook/dist/model/pca';
import * as tf from '@tensorflow/tfjs-core';
import { assert } from 'chai';
import { tensorEqual } from '@pipcook/datacook/dist/linalg';
import { PCAPredictor } from '../../src/model/pca-predictor';
...
describe('Principle Component Analysis', () => {
  ...
  it('iris decomposition', async () => {
    ...
    console.log('explained variance:', pca2.explainedVariance);
    console.log('explained varaince ratio:', pca2.explainedVarianceRatio);
    assert.isTrue(tensorEqual(irisExplainedVarianceRatio, tf.tensor(pca2.explainedVarianceRatio.data), 1e-4));
    assert.isTrue(tensorEqual(irisExplainedVariance, tf.tensor(pca2.explainedVariance.data), 1e-4));
  ...
  it('iris decomposition (correlation)', async () => {
    ...
    const modelJson = await pca.toJson();
    await pca2.fromJson(modelJson);
    console.log('explained variance:', pca2.explainedVariance.data);
    console.log('explained varaince ratio:', pca2.explainedVarianceRatio.data);
    assert.isTrue(tensorEqual(irisExplainedVarianceRatioCorr, tf.tensor(pca2.explainedVarianceRatio.data), 1e-2));
    assert.isTrue(tensorEqual(irisExplainedVarianceCorr, tf.tensor(pca2.explainedVariance.data), 1e-2));
  });
